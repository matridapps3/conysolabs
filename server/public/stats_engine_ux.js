...
function openComparator(analyses) {
  ...
  for (const a of analyses.slice(0, 4)) {
    ...
    if (a.chart_storage_key) {
      ...
      api.post(`/api/projects/${a.project_id}/storage/presign`, { key: a.chart_storage_key, ttl: 600 })
        .then(r => { if (r.url) img.src = r.url; }).catch(() => {});
      cell.append(img);
    }
...
const METRIC_PICKERS = {
  capability(s) {
    ...
    return [
      ...
      { label: 'Cpk',      value: fmt(s.cpk), klass: cpkClass(s.cpk) },
      { label: 'Pp',       value: fmt(s.pp),  klass: cpkClass(s.pp)  },
      { label: 'Ppk',      value: fmt(s.ppk), klass: cpkClass(s.ppk) },
    ];
  },
